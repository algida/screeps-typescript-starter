...
export class Log {
    ...
    public error(...args: any[]) {
        if (this.level >= LogLevels.ERROR) {
            console.log.apply(this, (this.buildArguments(LogLevels.ERROR) as any).concat([].slice.call(args)));
        }
    }

    public warning(...args: any[]) {
        if (this.level >= LogLevels.WARNING) {
            console.log.apply(this, (this.buildArguments(LogLevels.WARNING) as any).concat([].slice.call(args)));
        }
    }

    public info(...args: any[]) {
        if (this.level >= LogLevels.INFO) {
            console.log.apply(this, (this.buildArguments(LogLevels.INFO) as any).concat([].slice.call(args)));
        }
    }

    public debug(...args: any[]) {
        if (this.level >= LogLevels.DEBUG) {
            console.log.apply(this, (this.buildArguments(LogLevels.DEBUG) as any).concat([].slice.call(args)));
        }
    }
